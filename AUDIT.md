...
1. ~~**320 px déborde de 44 px sur toutes les pages.**~~ — **TRANCHÉ le 16 août : 375 px devient le plancher supporté. Aucun code.** Le menu hamburger est écarté. Mesures à 320 px qui ont motivé la décision : le menu de navigation `#cd-menu` **se replie déjà correctement** (largeur 0, `scrollWidth` 466, défilement horizontal via `cd-menu-defile`) et ne contribue **rien** au débordement. Les 44 px viennent d'ailleurs : `.in` padding 44 px + `.brand` 78 px + **`#cd-entete-session` 247 px qui ne se replie pas du tout** (3 icônes + FR/EN + bouton compte ; la Phase 1.5 T1 y avait déjà récupéré 51 px en repliant le prénom sous 430 px). Total requis 364 px pour 320 px disponibles. Un hamburger aurait donc restructuré la seule partie qui fonctionne, dans `auth.js` — partagé par 28 pages et gelé par la Partie 0.3 — au profit d'un parc d'appareils réduit à l'iPhone SE 1ʳᵉ génération (2016) et à l'iPhone 5S. Le plus petit iPhone vendu aujourd'hui fait 375 px, largeur où le débordement est **déjà nul**. Piste conservée pour après lancement, si jamais : réduire le padding de `.in` de 22 px à 6 px sous 360 px fait passer le débordement de 44 px à **28 px** (mesuré), et deux autres réductions (`gap` de `.droite`, espacement des icônes) fermeraient le reste — le tout dans l'exemption déjà accordée, sans changer destinations, ordre ni libellés.
2. ~~**`banniere` est vide en base.**~~ — **TRANCHÉ le 16 août : contrôle à l'approbation admin. Aucun code.** Vérification faite : le champ **existe déjà de bout en bout** — colonne `banniere text` dans `sql/01-auth-profiles.sql`, champ de saisie dans `profil.html` (l.204, étiqueté FR/EN), chargé l.693, enregistré l.768, affiché sur 8 pages dont la carte de contrat. Il est vide simplement parce qu'il est facultatif : **l'inscription ne collecte aucun champ de pharmacie** (`acces.html` demande prénom, nom, courriel, téléphone, mot de passe, consentement — rien d'autre) ; tout le profil pharmacie se remplit ensuite dans `profil.html`. Robert a écarté le rendre obligatoire dans `profil.html` (option A) et l'ajouter à l'inscription (option B) : le contrôle se fait **à la main au moment d'approuver la pharmacie**. La file de vérification le permet déjà — `sql/25-verification-queue.sql` l.123 compte `banniere` dans son score de complétude du profil, donc un profil sans bannière est visible dans la console admin sans rien coder. Raison d'être du champ (Robert) : la bannière dit au remplaçant **pour qui il travaille**.
