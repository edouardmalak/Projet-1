...
>
> **Remaining before any real money can move:** the **pharmacien** row still has `stripe_account_id` = null (sql/80 cleared the sandbox account). No connected account = no T-24h hold can be created. Live Connect onboarding must be redone from `profil.html` — that is step 6 below, and it now blocks Tests C and the mirror test.
