...
>
> ✅ **STEP 5 CLEARED — 2026-08-17. Stripe accepted the identity document.** Verified by Claude directly in the live dashboard: https://dashboard.stripe.com/connect/accounts/overview now shows **"Thanks for providing a valid identity document."** where the yellow "We need a valid identity document for EDOUARD ABDEL MALAK" banner used to be. The Connect overview page carries no blocking banner. On the Platform profile page, *Refunds and chargebacks liability acknowledgement* and *Ongoing seller compliance acknowledgement* both read **Completed August 17, 2026**; the leftover "Onboarding incomplete / View onboarding" text there is stale — clicking **View onboarding** presents no form and bounces straight back to Connect overview, i.e. Stripe has nothing further to ask. The "Connect setup almost complete — finish confirming your integration before creating your first live account" card on the Connected accounts page is the standard empty-state nudge shown while zero live connected accounts exist; it does not gate the API.
>
> **Remaining before any real money can move:** the **pharmacien** row still has `stripe_account_id` = null (sql/80 cleared the sandbox account). No connected account = no T-24h hold can be created. Live Connect onboarding must be redone — that is step 6 below, and it still blocks Test C and the mirror test.
