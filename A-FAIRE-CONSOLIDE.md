...
>
> **Every dashboard route to the questionnaire is currently broken (all tried 2026-08-17):** Connected accounts → *Create a live connected account* is disabled, its tooltip points to "setup" → that opens the Setup guide widget → *Verify your account* → *Activate payments* → loops to `/account/activated` ("Thanks for activating your account!", the page Robert pasted) → *Create your Stripe profile* stays locked. Platform profile → *View onboarding* → bounces to `/connect`, no form. Settings → Connect exposes only two read-only pages (Platform profile, Platform setup). Home carries no banner. `/settings/connect/platform-setup` shows all five integration selections correctly (direct charges; Stripe collects fees from sellers; Stripe responsible for losses; dashboard none; Stripe-hosted onboarding) but is the *recommendation* tool, not the binding profile.
>
> **Next action: Stripe support** — this is a Stripe-side state mismatch Robert cannot resolve from the UI. Ask them to surface or force-complete the Connect platform profile questionnaire for `acct_1Tz4YOReBKCY8Yl1`.
>
